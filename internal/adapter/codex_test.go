...
// TestCodexAdapterPreflightMissing verifies that Preflight returns an error
// when the codex binary is absent from PATH.
func TestCodexAdapterPreflightMissing(t *testing.T) {
	// Make sure there is no "codex" in PATH by using an isolated empty dir.
	emptyDir := t.TempDir()
	orig := os.Getenv("PATH")
...
// TestCodexAdapterPreflightWorkdirMissing verifies that Preflight fails for a
// non-existent working directory.
func TestCodexAdapterPreflightWorkdirMissing(t *testing.T) {
	buildFakeCodex(t, "#!/bin/sh\nexit 0\n")

	a := &CodexAdapter{}
	err := a.Preflight(context.Background(), "/nonexistent-codex-arc-test-workdir")
	if err == nil {
...
// TestCodexAdapterPreflightWorkdirNotWritable verifies that Preflight fails
// when the working directory is read-only.
func TestCodexAdapterPreflightWorkdirNotWritable(t *testing.T) {
	buildFakeCodex(t, "#!/bin/sh\nexit 0\n")
...
// TestCodexAdapterPreflightSuccess verifies that Preflight succeeds when
// codex is in PATH and workdir is accessible.
func TestCodexAdapterPreflightSuccess(t *testing.T) {
	buildFakeCodex(t, "#!/bin/sh\nexit 0\n")

	a := &CodexAdapter{}
	if err := a.Preflight(context.Background(), t.TempDir()); err != nil {
