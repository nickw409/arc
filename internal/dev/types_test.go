...
func TestDirectWorkflowLoads(t *testing.T) {
	data, err := resources.WorkflowBytes("direct")
	if err != nil {
		t.Fatalf("WorkflowBytes(\"direct\") error: %v", err)
	}
	wf, err := workflow.LoadBytes(data)
	if err != nil {
		t.Fatalf("LoadBytes error: %v", err)
	}
	if wf.EntryState != "execute" {
		t.Errorf("entry state = %q, want %q", wf.EntryState, "execute")
	}
	wantTerminal := []string{"complete", "blocked"}
	if !reflect.DeepEqual(wf.TerminalStates, wantTerminal) {
		t.Errorf("terminal states = %v, want %v", wf.TerminalStates, wantTerminal)
	}
	if len(wf.States) != 3 {
		t.Errorf("state count = %d, want 3", len(wf.States))
	}
}

func TestDirectWorkflowTransitions(t *testing.T) {
	data, err := resources.WorkflowBytes("direct")
	if err != nil {
		t.Fatalf("WorkflowBytes error: %v", err)
	}
	wf, err := workflow.LoadBytes(data)
	if err != nil {
		t.Fatalf("LoadBytes error: %v", err)
	}
	m := workflow.NewMachine(wf)
	next, err := m.NextState("execute", "")
	if err != nil {
		t.Fatalf("NextState(\"execute\", \"\") error: %v", err)
	}
	if next != "complete" {
		t.Errorf("NextState(\"execute\", \"\") = %q, want %q", next, "complete")
	}
}

func TestDirectWorkflowTerminal(t *testing.T) {
	data, err := resources.WorkflowBytes("direct")
	if err != nil {
		t.Fatalf("WorkflowBytes error: %v", err)
	}
...
func TestDirectWorkflowExecuteHasNoVerdicts(t *testing.T) {
	data, err := resources.WorkflowBytes("direct")
	if err != nil {
		t.Fatalf("WorkflowBytes error: %v", err)
	}
	...
	m := workflow.NewMachine(wf)
	verdicts := m.ValidVerdicts("execute")
	if verdicts != nil {
		t.Errorf("expected nil verdicts for execute state, got %v", verdicts)
	}
}

func TestDirectWorkflow_BlockedState(t *testing.T) {
	data, err := resources.WorkflowBytes("direct")
	if err != nil {
		t.Fatalf("WorkflowBytes error: %v", err)
	}
	...
	// Terminal states return Terminal constant, no outgoing transitions
	next, err := m.NextState("blocked", "")
	if err != nil {
		t.Fatalf("NextState(\"blocked\", \"\") error: %v", err)
	}
	if next != workflow.Terminal {
		t.Errorf("NextState(\"blocked\", \"\") = %q, want %q", next, workflow.Terminal)
	}
}

func TestDirectWorkflow_NextState_InvalidState(t *testing.T) {
	data, err := resources.WorkflowBytes("direct")
	if err != nil {
		t.Fatalf("WorkflowBytes error: %v", err)
	}
	...
	m := workflow.NewMachine(wf)
	_, err = m.NextState("nonexistent_state", "")
	if err == nil {
		t.Error("expected error for nonexistent state, got nil")
	}
}

func TestDirectWorkflow_IsTerminal_InvalidState(t *testing.T) {
	data, err := resources.WorkflowBytes("direct")
	if err != nil {
		t.Fatalf("WorkflowBytes error: %v", err)
	}
	...
	m := workflow.NewMachine(wf)
	if m.IsTerminal("nonexistent_state") {
		t.Error("expected IsTerminal(\"nonexistent_state\") to be false")
	}
}

func TestDirectWorkflow_ValidVerdicts_InvalidState(t *testing.T) {
	data, err := resources.WorkflowBytes("direct")
	if err != nil {
		t.Fatalf("WorkflowBytes error: %v", err)
	}
	...
	m := workflow.NewMachine(wf)
	verdicts := m.ValidVerdicts("nonexistent_state")
	if verdicts != nil {
		t.Errorf("expected nil for nonexistent state, got %v", verdicts)
	}
}

func TestDirectWorkflowLoadBytes_Invalid(t *testing.T) {
	_, err := workflow.LoadBytes([]byte("invalid: yaml: content:"))
	if err == nil {
		t.Error("expected error for invalid workflow YAML, got nil")
	}
}

// --- Prompt loading tests ---

func TestExecutePromptLoads(t *testing.T) {
	data, err := resources.PromptBytes("direct/execute.md")
	if err != nil {
		t.Fatalf("PromptBytes(\"direct/execute.md\") error: %v", err)
	}
	if len(data) == 0 {
		t.Error("expected non-empty prompt bytes")
	}
	if !strings.Contains(string(data), "reasoning-format") {
		t.Error("expected prompt to contain 'reasoning-format'")
	}
}

func TestDiscoveryPromptLoads(t *testing.T) {
	data, err := resources.PromptBytes("dev/discovery.md")
	if err != nil {
		t.Fatalf("PromptBytes(\"dev/discovery.md\") error: %v", err)
	}
	if len(data) == 0 {
		t.Error("expected non-empty prompt bytes")
	}
...
func TestArchitectPromptLoads(t *testing.T) {
	data, err := resources.PromptBytes("dev/architect.md")
	if err != nil {
		t.Fatalf("PromptBytes(\"dev/architect.md\") error: %v", err)
	}
	if len(data) == 0 {
		t.Error("expected non-empty prompt bytes")
	}
...
func TestCommonPartialsLoad(t *testing.T) {
	partials := []string{
	...
	for _, p := range partials {
		...
		if err != nil {
			t.Errorf("PromptBytes(%q) error: %v", p, err)
			continue
		}
		if len(data) == 0 {
			t.Errorf("PromptBytes(%q) returned empty bytes", p)
		}
	}
}

func TestDirectWorkflowInListWorkflows(t *testing.T) {
	workflows := resources.ListWorkflows()
	for _, w := range workflows {
		if w == "direct" {
			return
		}
	}
	t.Errorf("ListWorkflows() = %v, does not contain \"direct\"", workflows)
}
